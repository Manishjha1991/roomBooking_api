      ...
      db
        ...
        .toArray((err, result) => {
          ...
          if (err) {
            ...
            res.json({
              statusCode: 404,
              body: { result: result, status: "Room is already  booked" }
            });
            db.close();
          } else if (result) {
            console.log(result);
            res.json({
              statusCode: 200,
              body: { result: result, status: "Room is available for booking" }
            });
            db.close();
          }
        });
  ...
  MongoClient.connect(dbUrl, { native_parser: true }, (err, db) => {
    ...
    try {
      ...
      db
        ...
        .aggregate(
          { $unwind: "$reserved" },
          { $match: { "reserved.booking_date": new Date(booking_date) } },
          function(err, result) {
            console.log(result);
  ...
  MongoClient.connect(dbUrl, { native_parser: true }, (err, db) => {
    ...
    try {
      db
        ...
        .findOneAndUpdate(
          { room_id: room_id },
          { $pull: { reserved: { booking_id: booking_id } } },
          { new: true },
          (error, result) => {
            if (error) {
